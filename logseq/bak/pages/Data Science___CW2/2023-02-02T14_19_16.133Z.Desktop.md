...
- ### 1a [10 points]
   Describe the **aims** of the study and its stated **findings**. Write between 80-120 words.
	...
	- Answer:  {{renderer :wordcount_uzzxscxe}}
		...
		- It finds that the recruiter contact rate is 4-19% lower for immigrants and minority ethnic groups compared to those of the majority group and that this discrimination is exacerbated when recruiters get more tired as the day goes on. In terms of gender discrimination it finds that there is discrimination towards the gender whose field is not dominated by it however it averages out so that in the grand scheme of things there is seemingly no effect
-
-
- ### 1b [15 points].
  Describe the methods used to **collect and process** the data in the study. Be specific about what is being **measured and how**. Write between 230-280 words.
	...
	- Answer: {{renderer :wordcount_orilmhttp}}
		- Data was collected from Job-Room from 6 March 2017 to 31 December 2017 by the State Secretariat for Economic Affairs, who then anonymized and shared the data. It has 2 sections, the click data and the register data. 
		  
		  Certain pre-processing was done to the data. Firstly any collected by non-human systems (bots) was discarded. Any searches for recruits that did not specify search criteria was also not considered. Additionally, all searches that where another search was made less than 10 seconds later was not considered as well as all profile views where the profile was only glanced at for less than 2 seconds.
		  
		  Ethnicity was not something the data on jobseekers held intrinsically, instead it had to be estimated through 3 highly correlated markers: nationality, name and spoken languages. Nationality and Language are easy to pair with an ethnicity, however name had to go through the name-classification algorithm "Onolytics" that can be used to classify names based on ethnic origin. A jobseeker is then classified as either a 'typical' or 'atypical' member of their nationality's group main ethnic group, 'typical' members are nationals from that region that have a name that sounds like it comes from that region and speak a language of that region. The 'typical' members of these groups are the ones used to test for ethnic discrimination.
		  
		  //what? An outcome measure had to be established so they used whether or not so the binary data point of contact point was used.
- ### 1c [15 points]. 
